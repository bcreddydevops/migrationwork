...
kubectl exec -it $nexus6_backup -- /bin/bash -c "cksum /tmp/nexus6_backup.tar"
...
cat << EOF >> ~/nexusgen7.sh
    cd $HOME
	   ...
	   echo "Display the checksum value of nexus6_backup.tar"
	   kubectl exec -it `cat ~/nexus7podname` -- /bin/bash -c "cksum /opt/sonatype/sonatype-work/nexus3/nexus6_backup.tar"
