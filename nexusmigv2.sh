...
scp -r ~/nexus7podname ubuntu@gen7ip:/home/ubuntu/
...
nexus6cksum=$(kubectl exec -it $nexus6_backup -- /bin/bash -c "cksum /tmp/nexus6_backup.tar" | awk '{print $2}')

echo $nexus6cksum > ~/nexus6cksum
scp -r ~/nexus6cksum ubuntu@$gen7ip:/home/ubuntu/
...
cat << EOF >> ~/nexusgen7.sh
    cd $HOME
	   ...
	   echo "Display the checksum value of nexus6_backup.tar"
	   kubectl exec -it `cat ~/nexus7podname` -- /bin/bash -c "cksum /opt/sonatype/sonatype-work/nexus3/nexus6_backup.tar" | cut -d " " -f2 > nexus7cksum
	   
	   if [ "`cat ~/nexus6cksum`"=="`cat ~/nexus7cksum`" ]; then
	   echo "The contents of nexus6cksum and nexus7cksum are equal"
	   ...
	   echo " checksum values is not matched exiting the script"
...
ssh -t ubuntu@$gen7ip "rm -f nexusgen7.sh nexus6_backup.tar nexus6cksum nexus7cksum"
